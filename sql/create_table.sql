...
(
    id      varchar(100)    primary key comment '模型ID（OpenRouter格式）',
    name    varchar(200)    not null    comment '模型显示名称',
    description text        null    comment '模型描述',
    provider    varchar(100)    null comment '提供商',
    contextLength   int     null comment '上下文长度',
    modality    varchar(100)    null comment '模态，如 text+image->text',
    inputModalities     varchar(500)    null comment '输入模态（JSON数组字符串）',
    outputModalities    varchar(500)    null comment '输出模态（JSON数组字符串）',
    inputPrice  decimal(20, 10)  null comment '输入价格（每百万tokens，美元）',
    outputPrice     decimal(20, 10) null comment '输出价格（每百万tokens，美元）',
    ...
    totalTokens     bigint  default 0       not null comment '累计使用Token数量',
